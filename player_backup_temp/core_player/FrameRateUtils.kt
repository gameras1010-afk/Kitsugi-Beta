...
object FrameRateUtils {
    ...
    private fun sanitizeHeaders(headers: Map<String, String>?): Map<String, String> {
        ...
        raw.forEach { (key, value) ->
            ...
            if (k.isNotEmpty() && v.isNotEmpty() && !k.equals("Range", ignoreCase = true)) {
                sanitized[k.lowercase(java.util.Locale.ROOT)] = v
            }
        }
        return sanitized
    }
    ...
    fun getCachedFrameRate(url: String, headers: Map<String, String>): FrameRateDetection? {
        ...
        return synchronized(frameRateCache) {
            val cached = frameRateCache[key]
            if (cached != null) {
                val ageMs = System.currentTimeMillis() - cached.timestampMs
                if (ageMs > 60 * 60 * 1000L) { // 1 hour TTL
                    frameRateCache.remove(key)
                    null
                } else {
                    cached
                }
    ...
    private fun detectFrameRateWithExtractor(
        ...
        return try {
            ...
            val isDeclaredAmbiguous = declaredFrameRate != null && 
                    PlayerFrameRateHeuristics.isAmbiguousCinema24(declaredFrameRate)

            if (declaredFrameRate != null && isValidVideoFrameRate(declaredFrameRate) && !isDeclaredAmbiguous) {
                return FrameRateDetection(
            ...
            val probeResult = FrameRateDetection(
                raw = measured,
            ...
            if (declaredFrameRate != null && isValidVideoFrameRate(declaredFrameRate)) {
                val snappedDeclared = snapToStandardRate(declaredFrameRate)
                if (PlayerFrameRateHeuristics.shouldProbeOverrideTrack(
                        trackFrameRateRaw = declaredFrameRate,
                        trackFrameRate = snappedDeclared,
                        probeDetection = probeResult
                    )
                ) {
                    Log.i(TAG, "AFR: Overriding declared track frame rate ($declaredFrameRate fps) with probed NTSC film rate (${probeResult.raw} fps)")
                    return probeResult
                } else {
                    return FrameRateDetection(
                        raw = declaredFrameRate,
                        snapped = snappedDeclared,
                        videoWidth = detectedVideoWidth,
                        videoHeight = detectedVideoHeight
                    )
                }
