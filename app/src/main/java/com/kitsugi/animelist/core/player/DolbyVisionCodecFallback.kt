...
object DolbyVisionCodecFallback {
    ...
    /**
     * Builds a [MediaCodecSelector] that extends the default selector with a
     * profile-agnostic DV decoder fallback.
     *
     * When [convertToDv81Active] is true and the standard selector returns no
     * decoders for `video/dolby-vision`, [findDvDecodersIgnoringProfile] is
     * consulted so hidden DV8.1-capable hardware is still picked up.
     *
     * When [convertToDv81Active] is false this returns the standard selector
     * unchanged — zero overhead for non-DV content.
     */
    fun createDolbyVisionFallbackCodecSelector(
        convertToDv81Active: Boolean
    ): MediaCodecSelector {
        if (!convertToDv81Active) return MediaCodecSelector.DEFAULT
        return MediaCodecSelector { mimeType, requiresSecureDecoder, requiresTunnelingDecoder ->
            val standard = MediaCodecSelector.DEFAULT.getDecoderInfos(
                mimeType, requiresSecureDecoder, requiresTunnelingDecoder
            )
            if (standard.isNotEmpty()) return@MediaCodecSelector standard
            if (!mimeType.equals(DV_MIME, ignoreCase = true)) return@MediaCodecSelector emptyList()
            // Standard selector returned nothing for video/dolby-vision → try profile-agnostic fallback.
            val fallback = findDvDecodersIgnoringProfile()
            Log.d(TAG, "DV codec fallback: standard=0, fallback=${fallback.size} decoders")
            fallback
        }
    }
